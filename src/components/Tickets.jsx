    ...
    <div className="fixed inset-0 flex w-full h-screen overflow-hidden bg-[#0F0F13] text-white font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        ...
        <div className="pt-6 px-6 pb-4 flex justify-between items-center bg-[#0F0F13]">
          <div>
            <h2 className="text-xl font-black tracking-tight text-white uppercase">Tickets Guardados</h2>
            <p className="text-gray-500 text-[11px] mt-0.5">Gestión de Tickets POS</p>
          </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto px-6 py-2 custom-scroll whitespace-nowrap">
          <button className="px-5 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all bg-orange-500 text-white shadow-lg shadow-orange-500/30">
            TODOS LOS TICKETS
          </button>
        ...
        <div className="flex-1 overflow-y-auto custom-scroll px-6 py-4 bg-[#0F0F13]">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {tickets.map((ticket) => (
              <div key={ticket.id} className="bg-[#1c1c24] rounded-xl p-5 border border-gray-800 shadow flex flex-col gap-2 group relative">
